...
def index():
    ...
    pagination = Article.query.order_by(-Article.pdatetime).paginate(
    ...
    if uid:
        ...
        return render_template(
            "user/index.html", user=user, pagination=pagination, types=types
        )
    else:
...
def center():
    ...
    photos = Photo.query.filter(
        Photo.user_id == g.user.id, Photo.isdelete == False
    ).all()
    # 留言 查询自己并且没删除的
    messages = MessageBoard.query.filter(
        MessageBoard.post_user_id == g.user.id, MessageBoard.isdelete == False
    ).all()
    #                                               g是本次请求的对象,在before_app_request中获取了用户,这里不用在获取了
    return render_template(
        "user/center.html", user=g.user, types=types, photos=photos, messages=messages
    )
...
def message_board():
    ...
    # 查询留言
    messages = (
        MessageBoard.query.filter(MessageBoard.isdelete == False)
        .order_by(-MessageBoard.mdatetime)
