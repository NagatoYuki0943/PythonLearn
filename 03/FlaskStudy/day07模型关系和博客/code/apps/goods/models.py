...
class UserGoods(db.Model):
    id = db.Column(db.Integer, primary_key=True, autoincrement=True)
    user_id = db.Column(
        db.Integer, db.ForeignKey("user.id"), nullable=False, comment="用户外键"
    )  # laravel中不使用外键即可,直接绑定就行
    goods_id = db.Column(
        db.Integer, db.ForeignKey("goods.id"), nullable=False, comment="商品外键"
    )  # ForeignKey()里面是数据表的名字,必须为小写
